  ...
  return (
    <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-10 p-3 bg-gray-800 w-full">
      <div className="flex items-center justify-between w-full sm:w-auto">
        ...
        <button className="sm:hidden text-white text-2xl p-2" onClick={() => setMenuOpen(!menuOpen)}>
          <FaBars />
        </button>
      </div>

      <form className="bg-gray-50 flex gap-2 justify-between items-center rounded-xl p-2 w-full sm:w-auto mt-2 sm:mt-0">
        <input type="text" name="search" placeholder="search ..."  className="outline-none flex-1 min-w-0 bg-gray-50" id="search" />
        <FaSearch />
      </form>

      <ul className={`flex-col sm:flex-row flex sm:flex gap-4 sm:gap-10 items-center text-white text-xl px-3 mt-2 sm:mt-0 ${menuOpen ? 'flex' : 'hidden'} sm:flex`}>
